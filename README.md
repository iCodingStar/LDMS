...
## 特色介绍
 ...
 > 1、分模块构建项目，每个模块职责清晰，功能明确；
 > 
 > 2、后台管理系统、前台展示系统、静态文件系统分开管理，降低系统的压力，提高系统的可靠性，轻松实现分布式部署；
 > 
 > 3、项目主要实现了实验室设备的前台展示，后台管理，基本满足了实验室设备管理的需求；
 >
 > 4、整个项目给予SSM构建，是学习Java Web开发，SSM框架的良好范本。
...
## 如何贡献代码
 ...
 > ```
 > 
 > #### 3、进入到clone好的everyone-java-ldms目录里，执行以下命令，将你的副本地址加入到你的远端仓库中。
 > 
 > ```
...
#### 1、命名规范
 ...
 > 
 > 包名：由小写字母和点组成，禁止简写。示例：cn.cug.laboratory.controller
 > 
 > 常量名：大写字母与下划线组成，禁止简写。示例：CONNECTION_TIMEOUT，DEFAULT_SIZE
 > 
 > 数据库表名，字段名：小写字母加下划线组成。最多长度30个字母，允许适当使用简写。示例：user_ldms，comment_time
 >
 ...
 * 一定类型的类放在一定的包下，具体的规则如下
 ...
 > 数据库model：放在ldms-model下的cn.cug.laboratory.model.persistent包内

 > 传给前端的值对象：放在ldms-model下的cn.cug.laboratory.model.dto包内

 > 数据库的DAO类：放在ldms-dao下的cn.cug.laboratory.mapper包内

 > 业务逻辑service：接口放在ldms-service下的cn.cug.laboratory.service包内，实现类放在cn.cug.laboratory.service.impl包内

 > 工具类：放在ldms-common下的cn.cug.laboratory.common.utils包内

 > web框架通用的类：放在ldms-common的cn.cug.laboratory.common.web包内

 > ldms-{module}下的controller：放在ldms-{module}下的cn.cug.laboratory.{module}.controller包内，其中module为admin、api、cache、web其中一个
